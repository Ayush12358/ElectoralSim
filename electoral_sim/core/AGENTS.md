...
| File | Lines | Purpose |
|------|-------|---------|
| `model.py` | 849 | `ElectionModel(Model)` — chainable entry point |
| `config.py` | 143 | `Config` / `PartyConfig` dataclasses |
| `cli.py` | 408 | `main()` — run, batch, list-presets subcommands |
| `voter_generation.py` | 280 | `generate_voter_frame()`, `generate_party_frame()` |
