...
if st.button("Run Simulation", type="primary"):
    with st.spinner("Processing voters and ballots..."):
        if preset == "India (Lok Sabha)":
            ...
            fig_comp = plot_seats_vs_votes(
                results.vote_shares, {p: (s / 543) for p, s in results.seats.items()}
            )
            ...
            vote_shares = {
                p: s
                for p, s in zip(party_names, results["vote_counts"] / results["vote_counts"].sum())
            }
    ...
    st.image(
        "https://img.freepik.com/free-vector/election-campaign-concept_23-2148593457.jpg?w=1000",
