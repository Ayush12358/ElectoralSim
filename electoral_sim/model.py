...
class ElectionModel(Model):
    ...
    def _generate_voter_frame(self, n_voters: int) -> pl.DataFrame:
        """Generate initial voter DataFrame with random attributes."""
        rng = self.random
        
        return pl.DataFrame({
            "constituency": rng.integers(0, self.n_constituencies, size=n_voters),
            "ideology_x": np.clip(rng.normal(0, 0.3, n_voters), -1, 1),
            "ideology_y": np.clip(rng.normal(0, 0.3, n_voters), -1, 1),
            "party_id": rng.integers(0, self.n_parties, n_voters),
            "turnout_prob": rng.beta(5, 2, n_voters),  # Skewed toward higher
        })
