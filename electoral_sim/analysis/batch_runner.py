...
class BatchRunner:
    ...
    def run(self) -> pl.DataFrame:
        ...
        if self.verbose:
            print(f"Running {len(configs)} configurations × {self.n_runs_per_config} runs = {total_runs} total simulations")
    ...
    def _run_parallel(self, configs: list[dict]) -> list[dict]:
        ...
        with ProcessPoolExecutor(max_workers=self.n_jobs) as executor:
            ...
            iterator = tqdm(as_completed(futures), total=len(futures), desc="Simulations") if self.verbose else as_completed(futures)
