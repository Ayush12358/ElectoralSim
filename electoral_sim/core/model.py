...
class ElectionModel(Model):
    ...
    def __init__(
        ...
        from electoral_sim.behavior.voter_behavior import (
            BehaviorEngine, ProximityModel, ValenceModel, RetrospectiveModel
        )
    ...
    def _compute_utilities(self, **kwargs) -> np.ndarray:
        ...
        party_data = {
            ...
            'valence': valence,
            'incumbents': self.parties.df["incumbent"].to_numpy() if "incumbent" in self.parties.df.columns else np.zeros(len(self.parties), dtype=bool),
            'df': self.parties.df
        }
    ...
    def run_election(self, **kwargs) -> dict:
        ...
        if self.constituency_constraints:
            ...
            for cid, allowed_parties in self.constituency_constraints.items():
                ...
                excluded_indices = [i for i, name in enumerate(party_names) if name not in allowed_parties]
        ...
        if self.include_nota:
            ...
            nota_idx = self.parties.df.with_row_index("temp_idx").filter(pl.col("is_nota")).select("temp_idx").to_series().item(0)
            if self.electoral_system == "FPTP":
