...
def generate_network(
    n_agents: int,
    topology: Literal["barabasi_albert", "watts_strogatz", "erdos_renyi", "random_regular"] = "barabasi_albert",
    **kwargs,
) -> np.ndarray:
    """
        ...
        if model == "noisy_voter":
            ...
            if NUMBA_AVAILABLE and len(opinions) > 1000:
                ...
                new_opinions = noisy_voter_step(
                    opinions, self.adj_list, noise_rate, self.rng
                )
                
        elif model == "bounded_confidence":
            new_opinions = bounded_confidence_step(
                opinions, self.adj_list, epsilon, rng=self.rng
            )
        else:
